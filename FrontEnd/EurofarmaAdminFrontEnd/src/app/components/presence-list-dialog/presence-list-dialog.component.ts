...
export class PresenceListDialogComponent {
    ...
    constructor(
      ...
      @Inject(MAT_DIALOG_DATA) public data: {title: String, trainingId: number, trainingData: Training, filterByEmployeeRegistration: number | undefined },
    ...
    generatePDF() {
      ...
      const trainingData = [
        ...
        { label: "Tem quiz", value: this.data.trainingData.hasQuiz ? "Sim" : "Não" },
        { label: "Status", value: this.data.trainingData.opened ? "Aberto" : "Fechado" },
        { label: "Data de criação", value: this.data.trainingData.creation_date },
        { label: "Data de fechamento", value: this.data.trainingData.closing_date }
      ];
      ...
      let attendanceColumns = ['Nome', 'Sobrenome', 'RE', 'Assinatura', 'Data de registro'];
    
      if (this.data.trainingData.hasQuiz) {
        attendanceColumns = ['Nome', 'Sobrenome', 'RE', 'Assinatura', 'Data de\nregistro', 'Tentativas\nQuiz', 'Nota'];
      }
      ...
      (doc as any).autoTable({
        ...
        didDrawCell: (data: any) => {
          if (data.column.index === 3 && data.section != 'head' && this.dataSourceAttendanceList.data[data.row.index].signature) {
            const signature = this.dataSourceAttendanceList.data[data.row.index].signature;
            const imgWidth = 60;
            const imgHeight = 20;
            ...
            const imgYPos = data.cell.y + (data.cell.height - imgScaledHeight) / 2;
            doc.addImage(signature, 'PNG', imgXPos, imgYPos, imgScaledWidth, imgScaledHeight);
          }
        }
      });
